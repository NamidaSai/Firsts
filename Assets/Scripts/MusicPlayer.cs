...
public class MusicPlayer : MonoBehaviour
{
    ...
    [SerializeField] string followTrack = null;
    ...
    private void Start()
    {
        ...
        if (!string.IsNullOrEmpty(followTrack))
        {
            Play(followTrack);
        }
    }
    ...
    public void PlayWithoutLoop(string trackName)
    {
        Sound track = Array.Find(tracks, trackClip => trackClip.name == trackName);
        ...
        if (currentTrack != null && currentTrack.source.isPlaying)
        {
            StartCoroutine(SwitchTrackWithoutLoop(track));
            return;
        }
    ...
    private IEnumerator SwitchTrackWithoutLoop(Sound nextTrack)
    {
        currentTrack.source.loop = false;
        yield return new WaitWhile(() => currentTrack.source.isPlaying);
        nextTrack.source.Play();
        nextTrack.source.loop = false;
        currentTrack = nextTrack;
    }
}